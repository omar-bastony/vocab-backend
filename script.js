document.addEventListener("DOMContentLoaded", () => {
  ...
  function getSelectedLanguages() {
    return Array.from(langCheckboxes.querySelectorAll("input:checked")).map(
      (cb) => cb.value,
    );
  }
...
// --- Centralized Language Setup (Onboarding Welcome Pop-up Integrated) ---
  ...
  langMenuBtn.addEventListener("click", (e) => {
    ...
    closeAllDropdowns();
    if (isHidden) langDropdown.classList.remove("hidden");
  });
  grammarMenuBtn.addEventListener("click", (e) => {
    e.stopPropagation();
    const isHidden = grammarDropdown.classList.contains("hidden");
    closeAllDropdowns();
    if (isHidden) grammarDropdown.classList.remove("hidden");
  });
  materialMenuBtn.addEventListener("click", (e) => {
    e.stopPropagation();
    const isHidden = materialDropdown.classList.contains("hidden");
    closeAllDropdowns();
    if (isHidden) materialDropdown.classList.remove("hidden");
  });
    ...
    item.addEventListener("click", (e) => {
      ...
      if (type === "lesetexte") {
        ...
        data.headers.forEach((h) => {
          ...
          else if (h.includes("die") && !h.includes("Plural"))
            headerStyle = "color: #9C4150;";
  ...
  generateAiBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      alert(
        "Fehler beim Generieren. Bitte stellen Sie sicher, dass das Backend erreichbar ist.",
      );
    } finally {
  ...
  grammarModal.addEventListener("click", (e) => {
    if (e.target === grammarModal) closeGrammarModal();
  });
  ...
  suggestionText.addEventListener("click", () => {
    ...
    if (typeof clearInputBtn !== "undefined")
      clearInputBtn.classList.add("visible");
    const counter = document.getElementById("charCounter");
    if (counter) {
    ...
    if (currentMode === "sentence") {
      ...
      try {
        ...
        breakdownData.wordBreakdown.forEach((item) => {
          const tipHtml = item.grammarTip
            ? `<div class="wc-grammar">${item.grammarTip}</div>`
            : "";
          let caseBadgeHtml = "";
      ...
      document.getElementById("germanWordTitle").innerHTML =
      ...
      try {
        ...
        if (germanData.partOfSpeech === "noun" && germanData.article) {
          ...
          germanData.partOfSpeech === "verb" &&
          germanData.conjugationTips
        ) {
        ...
        document.getElementById("germanExample").innerText =
          `"${germanData.example}"`;
        ...
        translationGrid.innerHTML = "";
        const filteredTranslations = data.translations.filter((langData) =>
          selectedLangs.includes(langData.language),
        );
        ...
        fetch(
          `${BACKEND_URL}/api/image?germanWord=${encodeURIComponent(data.german.word)}&searchQuery=${encodeURIComponent(searchWord)}`,
        )
          .then((res) => res.json())
          .then((imgData) => {
            if (imgData.imageUrl) {
        ...
        if (germanData.example) {
          fetch(`${BACKEND_URL}/api/translate-sentence`, {
            ...
            .catch((e) => {
              ...
              document
                .querySelectorAll(".example-sentence .shimmer")
  ...
  function createTranslationCard(data) {
    ...
    const meaningsHtml =
      data.meanings.length > 1
        ? `<span class="multiple-meanings">(Auch: ${data.meanings.slice(1).join(", ")})</span>`
        : "";
    ...
    card
      .querySelector(".card-speak-btn")
      .addEventListener("click", () => playAudio(translatedWord, ttsCode));
    return card;
  }
});
